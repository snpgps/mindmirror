...
import { CalendarDays, Tag, ListChecks, Filter, X, Palette } from 'lucide-react';
...
export function MoodTimeline({ entries }: MoodTimelineProps) {
  ...
  const allActivityNamesForFilter = PREDEFINED_ACTIVITIES.map(a => a.name); // Consider adding custom activities if they become globally relevant
  ...
  const getCoreEmotionStyle = (moodWords: string[]): { name: string, className: string } => {
    if (!moodWords || moodWords.length === 0) return { name: "Mood", className: "bg-muted text-muted-foreground" };
    const firstWord = moodWords[0];
    for (const coreKey in EMOTION_HIERARCHY) {
      if (EMOTION_HIERARCHY[coreKey].name === firstWord) {
        // Use a toned-down version of the colorClass for badges for better readability
        const baseColor = EMOTION_HIERARCHY[coreKey].colorClass.split(' ')[0]; // e.g., bg-green-500
        // text color will be derived by shadcn's badge "secondary" variant or a custom one
        return { name: EMOTION_HIERARCHY[coreKey].name, className: `${baseColor.replace('-500', '-100')} text-${baseColor.split('-')[1]}-700 border-${baseColor.replace('-500', '-300')}` };
      }
    }
    return { name: firstWord, className: "bg-muted text-muted-foreground border-border" }; // Fallback
  };
                ...
                return (
                  <Card key={entry.id} className="bg-background/50 hover:shadow-md transition-shadow">
                    <CardHeader className="pb-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                           <Badge className={cn("text-lg px-3 py-1", coreEmotionStyle.className)}>
                    ...
                    <CardContent className="space-y-3">
                      {entry.moodWords && entry.moodWords.length > 1 && ( // Show secondary words if present
                        <div className="flex flex-wrap gap-2 items-center">
                          <Tag className="h-4 w-4 text-muted-foreground self-center" />
                          {entry.moodWords.slice(1).map(word => ( // Skip the first (core) word as it's in the header
